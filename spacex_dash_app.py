...
#Create dropdown labels from available launch sites
labels = [{'label': 'All Sites', 'value': 'ALL'}]
sites = pd.unique(spacex_df["Launch Site"])
labels.extend([{'label': site, 'value': site} for site in sites])
...
def get_scatter_chart(entered_site, mass_range):
    data = spacex_df[(spacex_df["Payload Mass (kg)"] >= mass_range[0])
    & (spacex_df["Payload Mass (kg)"] <= mass_range[1])]
    if entered_site == 'ALL':
        fig = px.scatter(data,
